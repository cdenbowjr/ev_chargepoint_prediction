...
imd_travel = pd.merge(imd_inc, travel.drop(['lad13cd', 'lad13nm'], axis=1), how='inner', on=['msoa11cd', 'msoa11nm'],
                      copy=True)
...
greater_man = ['Manchester', 'Bolton', 'Bury', 'Oldham', 'Rochdale', 'Salford', 'Stockport', 'Tameside', 'Trafford',
               'Wigan']
...
new_cols = list(full.iloc[:, :213].columns) + list(full.iloc[:, 214:-2].columns) + list(
    full.iloc[:, 213:214].columns) + list(full.iloc[:, -2:].columns)
