...
imd = pd.read_csv(
    "../../data/raw/socio_econ/File_7_-_All_IoD2019_Scores__Ranks__Deciles_and_Population_Denominators_3.csv",
    low_memory=False)
...
names = ['lsoa11cd', 'lsoa11nm', 'lad13cd', 'lad13nm', 'imd_score', 'imd_rank', 'imd_decile',
         ...
         'education_score', 'education_rank', 'education_decile', 'health_score', 'health_rank', 'health_decile',
         'crime_score', 'crime_rank', 'crime_decile', 'housebar_score', 'housebar_rank', 'housebar_decile',
         'livenv_score', 'livenv_rank', 'livenv_decile', 'idaci_score', 'idaci_rank', 'idaci_decile', 'idaopi_score',
         'idaopi_rank', 'idaopi_decile', 'chanyp_score', 'chanyp_rank', 'chanyp_decile',
         'adultskills_score', 'adultskills_rank', 'adultskills_decile', 'geo_bar_score', 'geo_bar_rank',
         'geo_bar_decile', 'widerbar_score', 'widerbar_rank', 'widerbar_decile',
         'indoor_score', 'indoor_rank', 'indoor_decile', 'outdoor_score', 'outdoor_rank', 'outdoor_decile', 'total_pop',
         'under16_pop', '16_59_pop',
...
imd.loc[:, 'imd_score':] = imd.loc[:, 'imd_score':].applymap(
    lambda x: eval(x.replace(",", "")) if type(x) == str else x)

# Extract MSOA data for each LSOA
lsoa2msoa = pd.read_csv("../../data/raw/geo_spatial/PCD_OA_LSOA_MSOA_LAD_MAY19_UK_LU.csv", encoding="ISO-8859-1",
                        low_memory=False)
