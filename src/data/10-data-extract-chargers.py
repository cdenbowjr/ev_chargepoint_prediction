...
# In[3]:
charge_points = pd.concat([address_info, connections, date_created, operator_info, status_type, usage_type, general],
                          axis=1)
...
charge_points.Postcode = charge_points.Postcode.apply(
    lambda x: " ".join([x[:-3].strip().upper(), x[-3:].strip().upper()]) if type(x) == str else x)
...
postcode_replace = ['306', '462', '1198', '1342', '1390', '1499', '1601', '1607', '1814', '1895', '2013', '2210',
                    '2214',
                    '2236', '2237', '2238', '2252', '2262', '2281', '2330', '2466', '2575', '2844', '3064', '3095',
                    '3309',
                    '3345', '3493', '3767', '3881', '4264', '4488', '4498', '4942', '5056', '5059', '5128', '5410',
                    '5736',
...
missing_post = missing_post.google_address.apply(
    lambda x: " ".join(x[-10:].split(" ")[-2:]) if len(x[-10:].split(" ")) > 1 else x)[3:]
...
postcode_data = postcode_data[
    ['lsoa11cd', 'lsoa11nm', 'msoa11cd', 'msoa11nm', 'ladcd', 'ladnm', 'pcds', 'region', 'oa11cd']].drop_duplicates(
...
# In[8]:
charge_points_lsoa = charge_points_map.groupby(
    ['lsoa11cd', 'lsoa11nm', 'msoa11cd', 'msoa11nm', 'ladcd', 'ladnm', 'region']).agg({'charge_points': sum})
charge_points_lsoa.to_csv("../../data/interim/lsoa/chargepoints_lsoa.csv", index=True)

# In[10]:
charge_points_msoa = charge_points_map.groupby(['msoa11cd', 'msoa11nm', 'ladcd', 'ladnm', 'region']).agg(
    {'charge_points': sum})
