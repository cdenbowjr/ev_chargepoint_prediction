...
part1a = df.loc[:, "health_score":"outdoor_score"].drop(
    ['housebar_score', 'livenv_score', 'idaci_score', 'idaopi_score', 'adultskills_score'], axis=1).columns
...
def get_col_names(df):
    ...
    return list(socio_econ1) + list(socio_econ1a) + list(pop_income) + list(transport) + list(car_own) + list(
        geo_spatial) + list(electricity) + list(metro) + list(charge_points)
