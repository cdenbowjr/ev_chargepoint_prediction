...
num_train_st = pd.Series([len(col['train_station']['venues']) for row, col in geo_spatial.iterrows()],
                         name='num_train_st')
...
num_supermarkets = pd.Series([len(col['supermarkets']['venues']) for row, col in geo_spatial.iterrows()],
                             name='num_supermarkets')

park_distance = pd.Series(
    [np.mean([x['location']['distance'] for x in col['parks']['venues']]) for row, col in geo_spatial.iterrows()],
    name='park_distance')
parking_distance = pd.Series(
    [np.mean([x['location']['distance'] for x in col['parking']['venues']]) for row, col in geo_spatial.iterrows()],
    name='parking_distance')
train_st_distance = pd.Series(
    [np.mean([x['location']['distance'] for x in col['train_station']['venues']]) for row, col in
     geo_spatial.iterrows()], name='train_st_distance')
gas_st_distance = pd.Series(
    [np.mean([x['location']['distance'] for x in col['gas_station']['venues']]) for row, col in geo_spatial.iterrows()],
    name='gas_st_distance')
hotel_distance = pd.Series(
    [np.mean([x['location']['distance'] for x in col['hotels']['venues']]) for row, col in geo_spatial.iterrows()],
    name='hotel_distance')
supermarkets_distance = pd.Series(
    [np.mean([x['location']['distance'] for x in col['supermarkets']['venues']]) for row, col in
     geo_spatial.iterrows()], name='supermarkets_distance')
...
geospatial_lsoa = pd.concat(
    [lsoa_json[['lsoa11cd', 'lsoa11nm']], num_parks, park_distance, num_parking, parking_distance, num_train_st,
     train_st_distance, num_gas_st, gas_st_distance, num_hotels, hotel_distance, num_supermarkets,
     supermarkets_distance], axis=1).fillna(0)
...
geospatial_msoa = pd.merge(lsoa2msoa, geospatial_lsoa, left_on=['lsoa11cd', 'lsoa11nm'],
                           right_on=['lsoa11cd', 'lsoa11nm'])
